...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        SDL_SetRenderDrawColor(renderer, 250+(20*(5-i)), 250+(20*(5-i)), 250+(20*(5-i)), 255);
        SDL_RenderDrawRect(renderer, &rect);
        SDL_RenderFillRect(renderer, &rect);
    }
...
static int initial_turn_left_counter = 7;
static int initial_up_counter = 4;
static int initial_up_counter2 = 4;
static int initial_turn_right_counter = 6;
static int initial_right_counter_bool = 0;
static int turn_left_counter = 6;
static int turn_right_counter = 6;
static int turn_left_bool = 0;
static int turn_right_bool = 0;
static int no_sensor_start_bool = 0;
static int step1_complete_bool = 0;
static int step2_complete_bool = 0;
static int initial_steps_complete_bool = 0;
static int counter_reset_counter = 0;



void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int back_left_sensor, int msec) {
    if (back_left_sensor == 0 && no_sensor_start_bool != 1) {
        no_sensor_start_bool = 1;
    }
    if (initial_turn_left_counter > 0 && initial_steps_complete_bool == 0 && no_sensor_start_bool == 1) {
        robot->direction = LEFT;
        initial_turn_left_counter -= 1;
        if (initial_turn_left_counter == 0) {
            step1_complete_bool = 1;
        }
    }

    if (step1_complete_bool == 1 && initial_up_counter > 0) {
        robot->direction = UP;
        initial_up_counter -= 1;
        if (initial_up_counter == 0) {
            step2_complete_bool = 1;
        }
    }

    if (step2_complete_bool == 1 && initial_turn_right_counter > 0 && front_left_sensor > 1 && front_right_sensor > 1) {
        initial_right_counter_bool = 1; 
    }

    if (initial_right_counter_bool == 1 && initial_turn_right_counter > 0) {
        robot->direction = RIGHT;
        initial_turn_right_counter -= 1;
        if (initial_turn_right_counter == 0) {
            initial_steps_complete_bool = 1;
        }
    }

    if (no_sensor_start_bool == 0 && initial_up_counter2 > 0) {
        robot->direction = UP;
        initial_up_counter2 -= 1;
        if (initial_up_counter2 == 0){
            initial_steps_complete_bool = 1;
        }
    }

    if (back_left_sensor == 0 && front_left_sensor == 0 && front_right_sensor == 0 && initial_steps_complete_bool == 1 && turn_left_bool == 0) {
        turn_left_bool = 1;
    }

    if (back_left_sensor == 0 && front_left_sensor > 2 && front_right_sensor > 2 && initial_steps_complete_bool == 1 && turn_left_bool == 0) {
        turn_left_bool = 1;
    }    

    if (turn_left_bool == 1 && turn_left_counter > 0) {
        robot->direction = LEFT;
        turn_left_counter -= 1;
    }

    if (turn_left_counter == 0) {
        turn_left_bool = 0;
    }

    if (back_left_sensor > 0 && turn_left_bool == 0) {
        turn_left_counter = 6;
    }

    if (back_left_sensor >= 1 && front_left_sensor > 1 && front_right_sensor > 1 && initial_steps_complete_bool == 1  && turn_right_bool == 0) {
        turn_right_bool = 1;
    }

    if (turn_right_bool == 1 && turn_right_counter > 0) {
        robot->direction = RIGHT;
        turn_right_counter -= 1;
        if (turn_right_counter == 0) {
            turn_right_bool = 0;
            turn_right_counter = 6;
        }
    }
}
